...
const GLchar* FragmentShaderSource = GLSL(440,
    ...
    // function prototypes
    vec3 CalcDirLight(DirLight light, vec3 vertexNormal, vec3 lightDirection);
    vec3 CalcPointLight(PointLight light, vec3 vertexNormal, vec3 vertexFragmentPos, vec3 lightDirection);
    vec3 CalcSpotLight(SpotLight light, vec3 vertexNormal, vec3 vertexFragmentPos, vec3 lightDirection);
    ...
    void main()
    {
        ...
        vec3 result = CalcDirLight(dirLight, norm, lightDirection);
        // phase 2: point lights
        for (int i = 0; i < NR_POINT_LIGHTS; i++)
            result += CalcPointLight(pointLights[i], norm, vertexFragmentPos, lightDirection);
        // phase 3: spot light
        result += CalcSpotLight(spotLight, norm, vertexFragmentPos, lightDirection);
    ...
    vec3 CalcDirLight(DirLight light, vec3 vertexNormal, vec3 lightDirection)
    {
        vec3 lightDirD = normalize(-light.direction);
        // diffuse shading
        float diff = max(dot(vertexNormal, lightDirD), 0.0);
        // specular shading
        vec3 reflectDir = reflect(-lightDirD, vertexNormal);
        float spec = pow(max(dot(lightDirection, reflectDir), 0.0), material.shininess);
        // combine results
    ...
    vec3 CalcPointLight(PointLight light, vec3 vertexNormal, vec3 vertexFragmentPos, vec3 lightDirection)
    {
        vec3 lightDirP = normalize(light.position - vertexFragmentPos);
        // diffuse shading
        float diff = max(dot(vertexNormal, lightDirP), 0.0);
        // specular shading
        vec3 reflectDir = reflect(-lightDirP, vertexNormal);
        float spec = pow(max(dot(lightDirection, reflectDir), 0.0), material.shininess);
        // attenuation
    ...
    {
        vec3 lightDirS = normalize(light.position - vertexFragmentPos);
        // diffuse shading
        float diff = max(dot(vertexNormal, lightDirS), 0.0);
        // specular shading
        ...
        float attenuation = 1.0 / (light.constant + light.linear * distance + light.quadratic * (distance * distance));
        // spotlight intensity
        float theta = dot(lightDirS, normalize(-light.direction));
...
void UCreateMesh(GLMesh& mesh)
{
    ...
    GLfloat verts[] = {
        ...
        // peek of pyramid
         0.0f,  0.5f,  0.0f,   0.0,  1.0,  0.0,  1.0f, 0.5f, 0.0f, 1.0f,  0.5f, 0.5f, // V0 Top: center vertex
        // sides of pyramid
         0.5f, -0.5f,  0.5f,   1.0, -1.0,  1.0,  0.0f, 1.0f, 0.5f, 1.0f,  1.0f, 0.0f, // V1 Side: Front Bottom-Right 
        -0.5f, -0.5f,  0.5f,  -1.0, -1.0,  1.0,  0.5f, 0.0f, 1.0f, 1.0f,  0.0f, 0.0f, // V2 Side: Front Bottom-Left
         0.5f, -0.5f, -0.5f,   1.0, -1.0, -1.0,  1.0f, 0.0f, 1.0f, 1.0f,  0.0f, 0.0f, // V3 Side: Back bottom-right
        -0.5f, -0.5f, -0.5f,  -1.0, -1.0, -1.0,  0.5f, 0.5f, 1.0f, 1.0f,  1.0f, 0.0f, // V4 Side: Back bottom-left
         // Base of pyramid. (Texture coordinates are different than sides)
         0.5f, -0.5f,  0.5f,   1.0, -1.0,  1.0,  0.0f, 1.0f, 0.5f, 1.0f,  1.0f, 1.0f, // V5 Base: Front-Right 
        -0.5f, -0.5f,  0.5f,  -1.0, -1.0,  1.0,  0.5f, 0.0f, 1.0f, 1.0f,  0.0f, 1.0f, // V6 Base: Front-Left
         0.5f, -0.5f, -0.5f,   1.0, -1.0, -1.0,  1.0f, 0.0f, 1.0f, 1.0f,  1.0f, 0.0f, // V7 Base: Back-right
        -0.5f, -0.5f, -0.5f,  -1.0, -1.0, -1.0,  0.5f, 0.5f, 1.0f, 1.0f,  0.0f, 0.0f  // V8 Base: Back-left
    };
    ...
    // TEXTURE
    glVertexAttribPointer(3, floatsPerUV, GL_FLOAT, GL_FALSE, stride, (void*)(sizeof(float) * (floatsPerVertex + floatsPerNormal + floatsPerColor)));
